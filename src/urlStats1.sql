...
FROM ( 
  SELECT regexp_extract(a.request, '.* (.*) (.*)?', 1) as url, SUM(a.size) as url_total_length, b.total_length as total_length 
  FROM tokenized_access_logs2 a,
    (SELECT SUM(size) AS total_length from tokenized_access_logs2) b
  GROUP BY regexp_extract(a.request, '.* (.*) (.*)?', 1), b.total_length
  ORDER BY url_total_length DESC
) sub;
